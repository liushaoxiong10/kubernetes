...
// 注册Know Type 资源类型，必须指定资源种类名称 GVK
...
// 判断指定的资源对象是否有属于UnversionedType类型
...
// 将传入的资源对象转化为目标资源版本，在版本转化之前，会将资源对象深复制一份后在执行转换操作，相当于安全的内存对象转换操作
...
// 与 ConvertToVersion 功能相同，但在转化过程中不会深复制资源对象，而是直接对原资源对象进行转化，但该操作是非安全的内存对象转换操作
